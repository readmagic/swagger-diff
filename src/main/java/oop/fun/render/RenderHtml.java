...
import oop.fun.utils.diff.model.*;
...
public class RenderHtml  {
    ...
    public RenderHtml(String title) {
        this.title = title;
    }


    public String render(SwaggerDiff diff,String oldestTime,String latestTime) {
        ...
        ContainerTag p_versions = p_versions(oldestTime, latestTime);
    ...
    public String renderHtml(ContainerTag ol_new, ContainerTag ol_miss, ContainerTag ol_changed, ContainerTag p_versions) {
        ContainerTag html = html().attr("lang", "en").with(
            head().with(
                ...
                link().withRel("stylesheet").withHref("/css/changeLog.css")
            ),
            body().with(
                header().with(h1(title)),
                div().withClass("article").with(
                    div_headArticle("版本", "versions", p_versions),
                    div_headArticle("新增", "new", ol_new),
                    div_headArticle("废弃", "deprecated", ol_miss),
                    div_headArticle("修改", "changed", ol_changed)
                )
            )
        );
    ...
    private ContainerTag p_versions(String oldVersion, String newVersion) {
        ...
        p.withText( oldVersion + " - " + newVersion );
        return p;
    }
