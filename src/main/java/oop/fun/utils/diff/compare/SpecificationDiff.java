...
import oop.fun.utils.diff.model.ChangedOperation;
    ...
    public static SpecificationDiff diff(Swagger oldSpec, Swagger newSpec) {
        ...
        sharedKey.stream().forEach((pathUrl) -> {
            ...
            sharedMethods.stream().forEach((method) -> {
                ...
                changedOperation.setAddConsumes(Objects.isNull(consumeDiff.getIncreased())? Lists.newArrayList() :consumeDiff.getIncreased());
                changedOperation.setMissingConsumes(Objects.isNull(consumeDiff.getMissing())?Lists.newArrayList():consumeDiff.getMissing());
